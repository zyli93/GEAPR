...
"""
    File to build user graph/item graph
...
def build_item_adj_ml(threshold, w):
    ...
    # process movie genre
    mg_mat = load_npz(parse_folder + "ml/mg.npz")
    mg_norm = normalize(mg_mat, axis=1, norm="l1")
    mg_sim = mg_norm.dot(mg_norm.T)

    # process movie tags
    mt_mat = load_npz(parse_folder + "ml/mt.npz")
